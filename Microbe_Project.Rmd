...
alpha_div <- plot_richness(TO.trades.bac.DCM, x = "Ocean_sea_regions", 
              measures = c("Shannon", "Chao1")) + 
  geom_boxplot(aes(fill = Ocean_sea_regions), alpha = 0.6) +
  ...
  labs(
  title = "Bacterial Alpha Diversity Across Ocean Basins",
...
plot_ordination(TO.trades.bac.DCM.rel.mean, GP.ord, type = "samples", color = "Mean_Salinity_PSU") +
  ...
  scale_color_viridis_c(name = "Salinity (PSU)") +
  theme_bw() +
  theme(
  ...
  labs(
    title = "Bacterial Community Structure in Trade Wind Biomes",
    subtitle = "Principal Coordinates Analysis (PCoA) using Bray-Curtis Dissimilarity",
    x = "PCoA1",
    y = "PCoA2"
  )
```
...
top_phyla <- names(sort(taxa_sums(PhylumGlommed), decreasing = TRUE)[1:10])
...
ggplot(data = psmelt(PhylumGlommed_filtered), 
  ...
  labs(
    ...
    subtitle = "Top 10 Most Abundant Phyla in Trade Wind Biomes at DCM",
    x = "Sample",
