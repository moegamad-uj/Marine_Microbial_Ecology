...
# Fit environmental variables from sample metadata (psv) onto NMDS ordination
# to identify factors (salinity, temperature, etc.) associated with bacterial
# community structure.
# Set up plotting parameters for larger, more spread out plots
op <- par(mfrow = c(1, 2), mar = c(5, 4, 4, 2) + 0.1)
on.exit(par(op))

# First NMDS plot with expanded limits and larger points
plot(vare_mds, display = "sites", main = "NMDS Ordination",
     xlim = c(-2, 2), ylim = c(-2, 2),
     cex = 1.5, # Increase point size
     type = "n") # Initially plot without points
points(vare_mds, display = "sites", cex = 1.5, pch = 19)

# Second plot with environmental vectors
plot(ef, p.max = 0.01, main = "Environmental Vector: Salinity",
     xlim = c(-2, 2), ylim = c(-2, 2),
     cex = 1.5) # Increase arrow and text size
