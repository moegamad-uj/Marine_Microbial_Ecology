...
if (taxa_are_rows(otu_table(TO.trades.bac.rel.mean))) {
...
# Fit environmental variables only if numeric_env has columns
if (ncol(numeric_env) > 0) {
  # Ensure rows match between NMDS and environmental data
  numeric_env <- numeric_env[rownames(otu_matrix), ]
  # Remove any remaining NA values
  complete_cases <- complete.cases(numeric_env)
  numeric_env <- numeric_env[complete_cases, ]
  nmds_scores <- scores(nmds_result)[complete_cases, ]
  envfit_result <- envfit(nmds_scores, numeric_env, perm = 999)
  # Set up plotting area with adjusted margins
  old_par <- par(no.readonly = TRUE)  # Save original par settings
  par(mar = c(5, 5, 4, 12))  # Increased right margin for legends
} else {
  warning("No numeric environmental variables found for envfit.")
}
...
region_colors <- setNames(colorRampPalette(brewer.pal(min(8, n_colors), "Dark2"))(n_colors), unique_regions)
points(nmds_result, pch = 21,
...
legend("topright",
       pt.bg = colorRampPalette(brewer.pal(8, "Dark2"))(length(unique_regions)),
       pch = 21,
       pt.bg = region_colors[unique_regions],
       cex = 0.8,
       title = "Ocean Regions",
       bty = "n",
       inset = c(-0.3, 0))
