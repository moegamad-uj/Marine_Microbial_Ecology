...
# Calculate buffer and bounds for zoom
buffer <- 30  # degrees
zoom_bounds <- list(
  long = c(min(npo_geo$Mean_Long) - buffer, max(npo_geo$Mean_Long) + buffer),
  lat = c(min(npo_geo$Mean_Lat) - buffer, max(npo_geo$Mean_Lat) + buffer)
)
...
ggplot() +
  ...
  coord_cartesian(
    xlim = zoom_bounds$long,
    ylim = zoom_bounds$lat
  ) +
