...
# Calculate buffer around points for zoom - increased for wider view
buffer <- 30  # increased from 10 to 30 degrees

# Create zoomed map with wider view
ggplot() +
  # Add world map
  geom_polygon(data = world_map, 
               aes(x = long, y = lat, group = group),
               fill = "lightgray", color = "darkgray", size = 0.2) +
  # Add NPO sample points
  geom_point(data = npo_geo, 
             aes(x = Mean_Long, y = Mean_Lat, color = Sample_label),
             size = 4, alpha = 0.8) +  # slightly smaller points for broader view
  # Add sample labels
  geom_text(data = npo_geo,
            aes(x = Mean_Long, y = Mean_Lat, label = Sample_label),
            vjust = -1.5, size = 3, fontface = "bold") +  # smaller text for better fit
  # Styling
  theme_bw() +
  theme(
    legend.position = "bottom",
  ...
  labs(
    title = "NPO Sample Locations - Regional View",
    subtitle = "TARA Oceans Bacterial Community Samples at DCM",
    x = "Longitude",
    y = "Latitude",
    color = "Sample"
  ) +
  scale_color_brewer(palette = "Set2") +
  # Set wider boundaries
  coord_cartesian(
    xlim = c(min(npo_geo$Mean_Long) - buffer, max(npo_geo$Mean_Long) + buffer),
    ylim = c(min(npo_geo$Mean_Lat) - buffer, max(npo_geo$Mean_Lat) + buffer)
  ) +
  # Add graticules with wider spacing
  scale_x_continuous(breaks = seq(floor(zoom_bounds$long[1]), 
                                ceiling(zoom_bounds$long[2]), 
                                by = 10)) +  # increased from 5 to 10
  scale_y_continuous(breaks = seq(floor(zoom_bounds$lat[1]), 
                                ceiling(zoom_bounds$lat[2]), 
                                by = 10))    # increased from 5 to 10

ggsave("plots/npo_regional_map.png", 
       width = 12, height = 8, dpi = 300)
```